...
const MyDropdown = ({style,text,required,tempdata,label, onSelect,type=1}) => {
  ...
  return (<View>
   ...
   { filterModal && (
      ...
      color={Colors.parrotGreenColor} size={26} />
               ...
               <FlatList
        data={data.sort((a, b) => type==2? a.name.localeCompare(b.name) : a.LoanTypeName.localeCompare(b.LoanTypeName) )}
        renderItem={({ item, index, separators }) => (
          <Pressable 
            style={{padding:12, borderBottomColor:'#f0f5f1', borderBottomWidth:1}}
            onPress = {()=>{
              setfilterModal(false)
              var matchedIndex = tempdata.map(function (obj) { return type==2?obj.name:obj.LoanTypeName }).indexOf(item.LoanTypeName);

              onSelect(item, matchedIndex )
            }}
            >
                <Text>{type==2?item.name:item.LoanTypeName}</Text>
            </Pressable>
        )}
      />
              </View>
